...
import edu.ktu.ds.lab2.Slegaityte.Phone;
import edu.ktu.ds.lab2.Slegaityte.PhoneGenerator;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private static ParsableSortedSet<Phone> phoneSet;
            ...
            public void handle(ActionEvent ae) {
                ...
                try {
                    ...
                    if (source.equals(mainWindowMenu.getMenus().get(0).getItems().get(0))) {
                        ...
                        saveFile();
                        //KsGui.ounerr(taOutput, MESSAGES.getString("notImplemented")); // Saving part here niggers
    ...
    private void pollLast(){
        ...
        KsGui.oun(taOutput, "Išrintas aukščiausias: " + phoneSet.pollLast().print());
        KsGui.setFormatStartOfLine(false);
        
        KsGui.oun(taOutput, phoneSet.toVisualizedString(tfDelimiter.getText()));   
    }
    private void treeHeight() {
        KsGui.setFormatStartOfLine(true);
        KsGui.oun(taOutput, "Medžio aukštis: " + phoneSet.treeHeight());
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void treeGeneration(String filePath) throws ValidationException {
        ...
        if (filePath == null) {
            carsArray = phoneGenerator.generateShuffle(sizeOfGenSet, sizeOfInitialSubSet, shuffleCoef);
            paneParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            phoneSet.load(filePath);
            carsArray = new Phone[phoneSet.size()];
            int i = 0;
            for (Object o : phoneSet.toArray()) {
                carsArray[i++] = (Phone) o;
            }
        ...
        phoneSet.clear();
        Arrays.stream(carsArray).forEach(phoneSet::add);
    ...
    private void treeAdd() throws ValidationException {
        KsGui.setFormatStartOfLine(true);
        Phone car = phoneGenerator.takePhone();
        phoneSet.add(car);
        paneParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
    ...
    private void treeRemove() {
        ...
        if (phoneSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(phoneSet.size());
            Phone car = (Phone) phoneSet.toArray()[nr];
            phoneSet.remove(car);
    ...
    private void treeIteration() {
        ...
        if (phoneSet.isEmpty()) {
            ...
            KsGui.oun(taOutput, phoneSet, MESSAGES.getString("setIterator"));
        }
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws ValidationException {
        switch (cmbTreeType.getSelectionModel().getSelectedIndex()) {
            case 0:
                phoneSet = new ParsableBstSet<>(Phone::new);
                break;
            case 1:
                phoneSet = new ParsableAvlSet<>(Phone::new);
                break;
            default:
                disableButtons(true);
    ...
    private void saveFile() throws IOException{
        ...
        BufferedWriter file = new BufferedWriter(new FileWriter(Ks.getDataFolder() + "\\save.txt"));
        
        for (Phone car : phoneSet) {
