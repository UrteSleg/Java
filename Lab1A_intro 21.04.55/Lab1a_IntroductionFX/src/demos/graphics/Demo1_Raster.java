...
public class Demo1_Raster extends BaseGraphics {
    ...
    public void drawRecursiveUpsideDown(double x, double y, double w, double h) { 
        if (h < 4) return;       // rekursijos pabaigos sąlyga
        gc.setStroke(randomColor());  
        gc.setFill(randomColor());   // parenkame spalvas ir linijos plotį
        gc.setLineWidth(h < canvasH/10? 0.7: 3);
        gc.fillOval(x, y, w, h); 
        gc.strokeOval(x, y, w, h); 
        drawRecursiveUpsideDown(x,     y + h/2, w/2, h/2);
        drawRecursiveUpsideDown(x+w/2, y + h/2, w/2, h/2);
    }   
    @Override
    public void createControls(){
        ...
        addButton("Circles down", action -> drawRecursiveUpsideDown(0, 0, canvasW, canvasH));
        addNewHBox();
    }
    @Override
