...
//        });
    ...
    public void createControls(){
        ...
        addButton("CustomLines", e -> {
        balls.clear();
        canvas.setOnMouseClicked(drawLine);
        });
    }
    private final EventHandler<MouseEvent> drawLine = e -> {
    ...
    nodes.add(ball); // Draws balls on the screen
    balls.add(ball);
    if(balls.size() == 2){
        Line line = new Line();  

        line.setStartX(balls.get(balls.size() - 2).getCenterX());
        line.setStartY(balls.get(balls.size() - 2).getCenterY());

        line.setEndX(balls.get(balls.size() - 1).getCenterX());
        line.setEndY(balls.get(balls.size() - 1).getCenterY());
