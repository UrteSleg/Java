...
import edu.ktu.ds.lab3.Slegaityte.Phone;
import edu.ktu.ds.lab3.Slegaityte.PhoneGenerator;
import edu.ktu.ds.lab3.Slegaityte.HashMapOA;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private final PhoneGenerator phonesGenerator = new PhoneGenerator();
...
//                KsGui.oun(taEvents, value.toString());
//            }
//            KsGui.oun(taEvents,"================================");
        }
    }
...
//        Book test = booksGenerator.getBook();
//        String id = booksGenerator.getBookId();
//        KsGui.oun(taEvents, "Ar yra verte: " + id + " <=> " + test.toString());
//        Book kek = map.putIfAbsent(id, test);
//        KsGui.oun(taEvents, kek == null ? "Ideta nauja reiksme" : "Verte jau egzistuoja: " + kek.toString());
//
//        // Atnaujinama lentele
...
//
//       // Book test = booksGenerator.getBook();
//        KsGui.oun(taEvents, "Ar turi verte: " + testBook.toString());
//        KsGui.oun(taEvents, map.containsValue(testBook));
//    }

    Phone testPhone = new Phone("TestBrand", "TestModel", 1999, 256);
    ...
    private void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            Phone[] phoneArray = phonesGenerator.generateShufflePhoneAndIds(sizeOfGenSet, sizeOfInitialSubSet);
            for (Phone c : phoneArray) {
                map.put(
                        phonesGenerator.getPhoneId(), //raktas
                        c);
            }
            map.put("TestingBook", testPhone);
...
//            }
    ...
    private void mapPut() {
        Phone car = phonesGenerator.getPhones();
        map.put(
                phonesGenerator.getPhoneId(), // Raktas
                car);
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            case 0:
                map = new ParsableHashMap<>(String::new, Phone::new, initialCapacity, loadFactor, ht);
                break;
...
//            // Užimtų maišos lentelės elementų skaičius procentais
            ...
            String.valueOf(map.numberOfEmpties())
