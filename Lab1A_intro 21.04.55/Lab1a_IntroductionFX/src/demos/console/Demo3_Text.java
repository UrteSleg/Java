...
void regexExamples(){
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            ta1.appendText("\\d+<>pirmas=1;(antras)>-22:trečias:(4568)333viskas:žodis" + nL);
        Pattern pat1 = Pattern.compile("-\\d+"); // Neigiamu skaiciu atpazinimas
        Pattern pat2 = Pattern.compile("\\p{IsLatin}+"); // Lotynisku raidziu atpazinimas
        Pattern pat3 = Pattern.compile("\\w*[ą-žĄ-Ž]\\w*"); // Lietuvisku raidziu atpazinimas
        Pattern pat4 = Pattern.compile("\\((.*?)\\)"); // Skliaustu atpažinimas
        Matcher m = pat3.matcher(ta1.getText());            // kita dalis - apdorojimui
